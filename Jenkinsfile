pipeline {
    ...
    stages {
        ...
        stage("Python") {
            ...
            steps {
                script {
                    sh "python3 python/delete-default-vpcs.py --mgmt_account_number=${env.MANAGEMENT_ACCOUNT} --account_number=${getAccountNumberFromStage()} --stage=${params.STAGE} --deployment_type=jenkins"
                    slackSend(
                            color: "good",
